...
contract ApeBotV3 is IAlphaLeakConstants {
    ...
    function ape(uint256[] memory data) internal {
        ...
        for (; callId < data.length;) {
            assembly {
		/* each call has this layout in data: [callInfo, eth value, callData... (up to callLength words)]*/
		/* callInfo is the 256 bit word at data[callId + 1] */
		/* the eth value to send in a call is stored at data[callId + 2] */
		/* bit-indexed layout */
		/* [:4] 'call type flag': delegatecall if 0x1, else call */
		/* [12:20] word-index of data to store call result in, 0x0 if no result */
		/* [20:36] callLength (in words): not including function signature
		   at the end of each iteration, callId is updated to 2 + this number */
		/* [36:68] callSignDataShiftResult (shifted to the left by 44 bits in a word):
		   store call data length worth of words starting with data[callId + 3] - 4 bytes in memory if not 0x0 */
		/* [68:92] gas used by call */
		/* [96:256] contract address to call */
                let callInfo := mload(add(data, mul(add(callId, 1), 0x20)))
		/* shift callInfo word to the right by 220 bits */
                ...
                case 0 {
		    /* if callSignDataShiftResult is 0 */
		    /* the bitlength of the calldata as determined in callInfo, no function signature */
                    callLength := mul(callLength, 0x20)
		    /* data[(callId + 3)*32: (callId + 3)*32 + callLength] (byte indexed, not word indexed) stores the calldata */
                    callSignDataShiftResult := add(data, mul(0x20, add(callId, 3)))
                    for { let i := 0 } lt(i, callLength) { i := add(i, 0x20) } {
			/* store calldata at the free memory pointer */
                        mstore(add(p, i), mload(add(callSignDataShiftResult, i)))
                    }
                }
                default {
		    /* if callDataSignShiftResult is not 0  */
		    /* store it at the free memory pointer */
                    mstore(p, callSignDataShiftResult)
		    /* the bitlength of callLength as determined in callInfo, plus the function signature */
                    callLength := add(mul(callLength, 0x20), 4)
		    /* data[(callId + 3)*32:(callId + 3)*32 + callLength] (byte indexed, not word indexed) stores the callData, including the function signature */
